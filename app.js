...
app.get(
  '/shift-member/checkpoint-submit/:location/:checkpointName/:shiftMember',
  async function (req, res) {
      ...
      if (patrolReport && patrolReport.status === 'Open') {
        ...
        if (cycleToUpdate) {
          ...
          if (checkpointToUpdate) {
            ...
            res.status(404).send('Successful update using QR scanner!');
          } else {
        ...
        res
          .status(404)
          .send('No patrol report found for the user or the status is closed!');
      }
    }
  }
);
