...
app.get("/forgot-password", function (req, res) {
    ...
    if (validationEmail === "is-valid") {
        ...
        res.render("forgot-password", {
            ...
            toastShow: "show",
            toastMsg:
                "Your email was not registered, please check your email again",
        });
    }
});
...
app.get("/reset-password/:customNameList", async function (req, res) {
    ...
    if (
        ...
        if (!existing) {
            ...
            if (result) {
                ...
                if (checkUser) {
                    const itemReports = await PatrolReport.find({}).sort({
                        date: -1,
                    });
                    const itemBMI = await PatrolReport.find({
                        location: "Baitul Makmur I",
                    ...
                    if (itemReports.length > 0) {
                        ...
                        res.render("patrol-report-view", {
                            ...
                            currentProfile: checkUser.profile,
                            itemReports:
                                "There is no patrol report submitted yet.",
                            totalReports: "0",
                            amountBMI: "0",
                            amountBMII: "0",
                            amountJM: "0",
...
app.post("/upload-case", async (req, res) => {
    if (!req.files || Object.keys(req.files).length === 0) {
        ...
        const newItemActivity = {
            ...
            type: "Upload Files",
            title:
                "Addes & uploaded " + Object.keys(req.files).length + " files",
                ...
                if (checkUser) {
                    ...
                    if (itemReports.length > 0) {
                        ...
                        res.render("case-report-view", {
                            ...
                            currentProfile: checkUser.profile,
                            itemReports:
                                "There is no case report submitted yet.",
                            totalReports: "0",
                            amountBMI: "0",
                            amountBMII: "0",
                            amountJM: "0",
        ...
        if (checkUser) {
            ...
            if (checkActivity.length > 0) {
                res.render("profile", {
                    ...
                    amountTotalReports:
                        checkPatrolReport.length + checkCaseReport.length,
    ...
    if (req.isAuthenticated()) {
        ...
        if (checkUser) {
            if (customListName === "submit-information") {
                ...
                // Full Name
                if (
                    fullNameRegex.test(fullName) === "false" ||
                    fullName === ""
                ) {
...
// SUBMIT
app.get("/duty-handover/submit", async function (req, res) {
    if (req.isAuthenticated()) {
        const currentUsername = req.session.user.username;
...
// DETAILS
app.get("/duty-handover/details", async function (req, res) {
    if (req.isAuthenticated()) {
        var currentUsername = req.session.user.username;
        const checkUser = await User.findOne({ username: currentUsername });
