...
// SUBMIT DATA USING QR
app.get('/shift-member/checkpoint-submit/:location/:checkpointName', async function (req, res) {
    if (req.isAuthenticated()) {
        const currentUsername = req.session.user.username;
        const checkUser = await User.findOne({ username: currentUsername });

        // get date today and current time
        ...
        // init the params from the link
        const location = _.startCase(
            req.params.location.replace(/-/g, ' ')
        );
        ...
        if (checkUser) {
            ...
            const patrolReport = await PatrolReport.findOne({
                staff: { $in: [checkUser.fullname] },
                location: location, // Add additional conditions if needed
                date: dateToday
            });

            if (patrolReport) {
                // Find the relevant cycle based on your logic and checkpointName
                const cycleToUpdate = patrolReport.shiftMember.cycle.find(
                    (cycle) =>
                ...
                if (cycleToUpdate) {
                    ...
                    const currentTime = parseInt(time.replace(/:/g, ''), 10) * 100;
                    ...
                    const isWithinTimeSlot = currentTime >= parseInt(startTime, 10) && currentTime <= parseInt(endTime, 10);
                    ...
                    if (isWithinTimeSlot) {
                        ...
                        // Add any additional logic or response here if needed
                        res.redirect('/');
                    } else {
                ...
                res.status(404).send("No patrol report found for the user.");
            }
